...
import React, { JSX, KeyboardEvent } from 'react';
...
export const CancelableFormControl = ({
  ...
  return (
    <div
      className={classes('d-flex align-items-center position-relative', formControlProps.className)}
    ...
    >
      ...
      {onCancel && !!value && value.toString().length > 0 && (
        <div
          className="p-2 d-flex align-items-center text-secondary cursor-pointer position-absolute end-0"
          onClick={() => {
            if (onCancel) {
              onCancel();
            }
